...
export const PII_PATTERNS: PiiPattern[] = [
  ...
  },
  {
    ...
    priority: 3,
    description:
      "Срок действия карты MM/YY или MM/YYYY — только через слеш без контекста, или с ключевым словом через дефис",
    ...
    regex:
      /(?:срок|действует\s+до|expir(?:y|es?)|valid\s+(?:thru|until|through)|exp\.?)[:\s]+(?:0[1-9]|1[0-2])\s*[/-]\s*(?:\d{2}|\d{4})\b|\b(?:0[1-9]|1[0-2])\/(?:\d{2}|\d{4})\b/gi,
  },
  {
    tokenName: "СЧЁТ_РФ",
    group: "finance",
...

  {
    ...
    regex:
      /\b\d{6}\s*,\s*[а-яА-ЯёЁ\s]+,\s*(?:ул\.|улица|пр\.|проспект|пер\.|переулок|бульвар|шоссе|набережная|площадь)\s+[а-яА-ЯёЁ\s\d]+,\s*(?:д\.?\s*\d+[а-яА-ЯёЁ]?)(?:,\s*(?:кв|оф|стр|корп)\.?\s*\d+)*/gi,
  },
  {
    tokenName: "УЛИЦА",
    group: "address",
...

  {
    ...
    regex:
      /\b(?:[А-ЯЁ][а-яё]{1,29}\s){1,2}[А-ЯЁ][а-яё]{1,29}(?:ич|вна|вич|овна|евна|евич|ович)?\b/g,
  },
  {
  ...
  },
  {
    ...
    regex:
      /(?:работаю в|работаю на|место работы|employer|работодатель)[:\s]+[А-ЯЁа-яёA-Za-z0-9\s"«»\-.]{3,80}/gi,
  },
...

  {
    ...
    regex:
      /(?:живу|нахожусь|прописан[аы]?|зарегистрирован[аы]?|родом из|переехал[аи]?\s+(?:в|из)|нахожусь\s+в|нахожусь\s+на|базируюсь\s+в|базируемся\s+в)\s+(?:в\s+|из\s+)?([А-ЯЁа-яёA-Za-z][А-ЯЁа-яёA-Za-z\s-]{2,60})/gi,
  },
...
export const SORTED_PATTERNS: PiiPattern[] = [...PII_PATTERNS].toSorted(
