...
export const PII_PATTERNS: PiiPattern[] = [
  ...
  },
  {
    ...
    // Только после явных индикаторов мессенджера или контакта, чтобы не захватывать системные имена агентов
    regex: /(?:telegram|tg|телеграм|whatsapp|вотсап|viber|вайбер|написать|свяжись|ник(?:нейм)?|контакт|логин)[:\s\-]+@[a-zA-Z][a-zA-Z0-9_]{4,31}\b/gi,
  },
  {
    tokenName: 'SKYPE',
    group: 'contacts',
  ...
  // ── РАЙОНЫ И МИКРОРАЙОНЫ (priority 117) ──────────────────────────────────
  {
    ...
    regex: /(?:мкр\.?|микрорайон|квартал|кв-л|жилой\s+массив|пос\.?|посёлок|деревня|дер\.?|село|станица|аул|хутор|ж\/м)\s+[«"]?[А-ЯЁа-яёA-Za-z0-9][А-ЯЁа-яёA-Za-z0-9\s\-\.]{2,60}[»"]?/gi,
  },
